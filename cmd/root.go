...
func init() {
	...
	rootCmd.Flags().StringVarP(&fUserAgent, "user-agent", "U", "", "Custom User-Agent header for HTTP requests")
...
loop:
	for _, user := range usernameList {
		...
		for _, pass := range passwordList {
			check := attemptLogin(client, user, pass, fAccountID, fDelay, fUserAgent, 1)
...
func attemptLogin(client *retryablehttp.Client, username string, password string, accountID string, delay int, userAgent string, bfSleepRounds int) ReturnStatus {
	...
	// create request with custom user agent if provided
	req, reqErr := retryablehttp.NewRequest("POST", signinURL, strings.NewReader(params.Encode()))
	if reqErr != nil {
		log.Printf("(%s)\t[!] ERROR:\tRequest Creation Failure. \tMessage: %s", username, reqErr.Error())
		return CONNFAIL
	}
	...
	if err != nil {
		...
		if resp.StatusCode == 429 {
			...
			return attemptLogin(client, username, password, accountID, delay, userAgent, 1)
		}
		...
		if loginResponse.State == "SUCCESS" {
			...
			if strings.Contains(loginResponse.Properties.Text, "many invalid passwords have been used") {
				...
				return attemptLogin(client, username, password, accountID, delay, userAgent, (bfSleepRounds + 1))
			}
			if fVerbose {
